...
HandleEnemyMonFainted: ; 3c525 (f:4525)
	...
	ld a, [wIsInBattle]
	dec a
	jr z, .giveFood ; handle food and whatnot
...
.skipReplacingBattleMon
	...
	jp MainInBattleLoop
.giveFood
	ld bc, $0 ; which function to use
	ld a, [wEnemyMonSpecies]
	ld d, a
	ld a, [wBattleMonSpecies]
	cp d ; same species?
	jr nz, .speciesDoesNotMatch
	ld c, $3 ; hardcode
	jr .executeHealFunc
.speciesDoesNotMatch
	...
	ld a, [de]
	cp [hl]
	jr nz, .firstTypesDoNotMatch
	inc c
.firstTypesDoNotMatch
	inc de
	inc hl
	ld a, [de]
	cp [hl]
	jr nz, .executeHealFunc
	inc c
.executeHealFunc
...
AfterBattleHealJumptable:
	dw AfterBattle_HealSomeHP
	dw AfterBattle_HealFullHP
	dw AfterBattle_HealPP
	dw AfterBattle_HealBothPPAndHP
	
AfterBattleHealTextPointers:
	dw GotFoodText
	dw GotSuperFoodText
	dw GotMilkText
	dw GotSuperFoodAndMilkText
...
GotSuperFoodAndMilkText:
	TX_FAR _GotSuperFoodAndMilkText
	db "@"
	
AfterBattle_GetMaxHP:
	push hl
...
AfterBattle_HealSomeHP:
	call AfterBattle_GetMaxHP
	call BattleRandom
	and $1f ; 32
	add (50 - 15) ; get range between -15 and 16
	ld [H_MULTIPLIER], a
	xor a
	ld [H_MULTIPLICAND], a
	ld a, d
	...
	call Divide ; dividend is already in H_DIVIDEND, shared value with H_PRODUCT
	
	ld a, [H_QUOTIENT+2]
	ld d, a
	ld a, [H_QUOTIENT+3]
	ld e, a ; de = food value

	push hl
	inc hl
	ld a, [hli]
	ld l, [hl]
	ld h, a ; hl = current HP
	add hl, de
	ld b, h
	ld c, l ; bc = modified HP
	pop hl
...
AfterBattle_HealBothPPAndHP:
	call AfterBattle_HealPP
; fallthrough
AfterBattle_HealFullHP:
	call AfterBattle_GetMaxHP
HealPokemonWithFood:
	inc hl
	ld a, d
...
AfterBattle_HealPP:
	...
	add hl, bc
	ld a, wPartyMon1Moves - wPartyMon1
	add e
	ld e, a
	jr nc, .noCarry
	inc d
